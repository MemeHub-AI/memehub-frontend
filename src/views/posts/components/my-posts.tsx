...
export const MyPosts = ({postObj}: postsPro) => {
    ...
    const imgs = ['/favicon.ico', '/images/ai.jpg', '/images/ai.jpg', '/images/ai.jpg']
    return (
        <div>
            {posts.map((p, i) => (
                <div className={cn('flex flex-col py-1')}>
                    <div className="flex items-center">
                        <div className='shrink-0'>
                            <Image className="inline-block h-10 w-10 rounƒded-full ml-1" src="/favicon.ico" alt="" width={50} height={50} />
                        </div>
                        <div className="ml-3 flex max-lg:justify-between w-[100%] items-center">
                            <div>
                            ...
                            {p.isLaunched ?
                                (p.isDetailed && <span className='bg-purple-500 rounded-md p-1 text-white lg:ml-[155px] font-bold'>
                                    {t('started')}
                                </span>) : 
                    ...
                    {!p.isDetailed && (p.isLaunched ?
                        <div className="ml-14 relative border border-solid border-yellow-600 text-yellow-500 rounded-md py-1 mt-2 lg:w-[400px]">
                    ...
                    <div className="pl-14 mt-2">
                        ...
                        {/* 判断已发射或者补全信息就可以显示 */}
                        {(!p.isLaunched || p.isDetailed) && <TokenBlock type={postObj.type}/>}
                        <Imgs imgs={imgs} className='mt-3 mr-1' />
                        <InteractiveList className='py-1' />
                        <Progress value={60} className='mb-3 text-black h-4 lg:w-[500px] rounded-sm' indicatorClass="bg-red-500">60%</Progress>
                    </div>
                    {i !== posts.length - 1 && <div className={cn('  bg-gray-400 h-[0.7px] mr-[-0.7rem] lg:w-[600px] pl-[-5px]  ml-[-1.5rem] sm:mr-[-1.4rem]')}></div>}
                </div>
            ))}
        </div>
    )
